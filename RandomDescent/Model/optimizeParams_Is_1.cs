...
{
	public class OptimizeParams_Is_1 : IOptimize, IVAX, IInaccuracy
	{
		...
		public OptimizeParams_Is_1(double[] I, double[] U, double Is, double f, double R, double IKF, double[] FPar)
		{
			...
			this.IsPar = new OptimizeParams(new double[] { 1, 1, 1 });
			...
			c = CalculationError(Is, f, R, IKF, this.IsPar.CurrentValue);
		...
		public void DoOptimize(int nStep)
		{
			...
			for (double i = 0; i < nStep - 1; i++)
			{
				...
				S = CalculationError(Is.GetNewValue(), f.GetNewValue(), R.GetNewValue(), IKF.GetNewValue(), IsPar.GetNewValue());
		...
		double CalculationError(double Is, double f, double R, double IKF, double[] IsPar)
		{
			double S = 0;
			double parIs = 0;
			for (int j = 0; j < I.Length; j++)
			{
				...
				VD = Math.Log((Math.Pow(I[j], 2) + Math.Sqrt(Math.Pow(I[j], 4) + 4 * Math.Pow(I[j], 2) * Math.Pow(IKF, 2))) / (2 * IKF * parIs) + 1) * f;
		...
		{
			return Is * (1 + IsPar[1] * U * U + IsPar[1] * U * U * U)/ (1 + IsPar[2] * U * U);
		}
		...
		private void InitVAX()
		{
			...
			for (int j = 0; j < U.Length; j++)
			{
				...
				UU_[j] = Math.Log((Math.Pow(I[j], 2) + Math.Sqrt(Math.Pow(I[j], 4) + 4 * Math.Pow(I[j], 2) * Math.Pow(IKF.Value, 2))) / (2 * IKF.Value * parIs) + 1) * f.Value + R.Value * I[j];
			}
		}
		public double[] GetI()
		{
		...
		public double[] InaccuracyOfCUrrent()
		{
			...
			for (int i = 0; i < I.Length; i++)
			{
				...
				VD = _VD(U[i], parIs, f.Value, IKF.Value, R.Value, VD);
				I_err[i] = I[i] - parIs * (Math.Exp(VD / f.Value) - 1) * Math.Sqrt(IKF.Value / (IKF.Value + parIs * (Math.Exp((VD) / f.Value) - 1)));
		...
		public double[] InaccuracyOfVoltage()
		{
			...
			{
				parIs = CalcIs(Is.Value, IsPar.Value, U[i]);
				U_err[i] = U[i] - Math.Log((Math.Pow(I[i], 2) + Math.Sqrt(Math.Pow(I[i], 4) + 4 * Math.Pow(I[i], 2) * Math.Pow(IKF.Value, 2))) / (2 * IKF.Value * parIs) + 1) * f.Value - R.Value * I[i];
